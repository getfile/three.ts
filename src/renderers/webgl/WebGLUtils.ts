...
class WebGLUtils
{
	...
	convert( p )
	{
		...
		if ( p === RepeatWrapping ) return this.gl.REPEAT;
		if ( p === ClampToEdgeWrapping ) return this.gl.CLAMP_TO_EDGE;
		if ( p === MirroredRepeatWrapping ) return this.gl.MIRRORED_REPEAT;

		if ( p === NearestFilter ) return this.gl.NEAREST;
		if ( p === NearestMipMapNearestFilter ) return this.gl.NEAREST_MIPMAP_NEAREST;
		if ( p === NearestMipMapLinearFilter ) return this.gl.NEAREST_MIPMAP_LINEAR;

		if ( p === LinearFilter ) return this.gl.LINEAR;
		if ( p === LinearMipMapNearestFilter ) return this.gl.LINEAR_MIPMAP_NEAREST;
		if ( p === LinearMipMapLinearFilter ) return this.gl.LINEAR_MIPMAP_LINEAR;
		...
		if ( p === AddEquation ) return this.gl.FUNC_ADD;
		if ( p === SubtractEquation ) return this.gl.FUNC_SUBTRACT;
		if ( p === ReverseSubtractEquation ) return this.gl.FUNC_REVERSE_SUBTRACT;

		if ( p === ZeroFactor ) return this.gl.ZERO;
		if ( p === OneFactor ) return this.gl.ONE;
		if ( p === SrcColorFactor ) return this.gl.SRC_COLOR;
		if ( p === OneMinusSrcColorFactor ) return this.gl.ONE_MINUS_SRC_COLOR;
		if ( p === SrcAlphaFactor ) return this.gl.SRC_ALPHA;
		if ( p === OneMinusSrcAlphaFactor ) return this.gl.ONE_MINUS_SRC_ALPHA;
		if ( p === DstAlphaFactor ) return this.gl.DST_ALPHA;
		if ( p === OneMinusDstAlphaFactor ) return this.gl.ONE_MINUS_DST_ALPHA;

		if ( p === DstColorFactor ) return this.gl.DST_COLOR;
		if ( p === OneMinusDstColorFactor ) return this.gl.ONE_MINUS_DST_COLOR;
		if ( p === SrcAlphaSaturateFactor ) return this.gl.SRC_ALPHA_SATURATE;
