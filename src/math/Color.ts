...
class Color
{
	...
	setHSL( h: number, s: number, l: number ): Color
	{
		...
		{
			let p: number = l <= 0.5 ? l * ( 1 + s ) : l + s - ( l * s );
			let q: number = ( 2 * l ) - p;
	...
	{
		let r: number = this.r, g: number = this.g, b: number = this.b;
	...
	{
		// h,s,l ranges are in 0.0 - 1.0
		let hsl: HSL = optionalTarget || { h: 0, s: 0, l: 0 };
		let r: number = this.r, g: number = this.g, b: number = this.b;
		...
		let lightness: number = ( min + max ) / 2.0;
