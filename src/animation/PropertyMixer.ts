...
class PropertyMixer
{
	...
	// mix functions
	private _select( buffer, dstOffset: number, srcOffset: number, t: number, stride: number )
	{
		if ( t >= 0.5 )
		{
			for ( let i = 0; i !== stride; ++i )
	...
	private _slerp( buffer, dstOffset: number, srcOffset: number, t: number )
	{
		Quaternion.slerpFlat( buffer, dstOffset, buffer, dstOffset, buffer, srcOffset, t );
	}

	private _lerp( buffer, dstOffset: number, srcOffset: number, t: number, stride: number )
	{
		let s = 1 - t;
		for ( let i = 0; i !== stride; ++i )
		{
